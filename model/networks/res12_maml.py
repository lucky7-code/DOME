...
class ResNetMAMLUS(nn.Module):
    ...
    def __init__(self, block=BasicBlock, keep_prob=1.0, avg_pool=True, drop_rate=0.1, dropblock_size=10):
        self.inplanes = 3
        super(ResNet12_FSL_FULL, self).__init__()
...
class ResNet12_FSL_FULL(nn.Module):

    def __init__(self, args, way, block=BasicBlock, keep_prob=1.0, avg_pool=True, drop_rate=0.1, dropblock_size=10):
        self.inplanes = 3
        super(ResNet12_FSL_FULL, self).__init__()
        if args.dataset == 'CUB':
            dims = [64, 128, 256, 512]
            ...
            dims = [64, 160, 320, 640]
        self.layer1 = self._make_layer(block, dims[0], stride=2, drop_rate=drop_rate)
        self.layer2 = self._make_layer(block, dims[1], stride=2, drop_rate=drop_rate)
        self.layer3 = self._make_layer(block, dims[2], stride=2, drop_rate=drop_rate, drop_block=True, block_size=dropblock_size)
        self.layer4 = self._make_layer(block, dims[3], stride=2, drop_rate=drop_rate, drop_block=True, block_size=dropblock_size)
        self.fc = nn.Linear(dims[3], way)
        if avg_pool:
            self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.keep_prob = keep_prob
        self.keep_avg_pool = avg_pool
    ...
    def forward(self, x, params = None, is_embedding=False):
        if params is None:
            params = OrderedDict(self.named_parameters())
...
class ResNet12_FSL(nn.Module):

    def __init__(self, args, block=BasicBlock, keep_prob=1.0, avg_pool=True, drop_rate=0.1, dropblock_size=10):
        self.inplanes = 3
        super(ResNet12_FSL, self).__init__()
        if args.dataset == 'CUB':
            dims = [64, 128, 256, 512]
        else:
            dims = [64, 160, 320, 640]
...
class Classifer(nn.Module):
    def __init__(self, dim, class_num):
        ...
        self.fc = nn.Linear(dim, class_num)
